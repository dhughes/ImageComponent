...
<cfcomponent displayname="Image" hint="I am a CFC which can be used to create and edit images." output="no">
	...
	<!--- readFromBinary --->
	<cffunction name="readFromBinary" access="public" output="false" returntype="void" hint="I create a new image from raw binary data.">
		<cfargument name="data" hint="I am the base 64 data which contains the image." required="yes" type="binary" />
		<cfargument name="mode" hint="I am an optional mode to convert the image to when reading.  Options are:Grayscale, RGB, ARGB, BGR, INDEXED, BINARY" default="" required="no" type="string" />
		<cfset var ImageReader = 0 />
		<cfset var InputStream = CreateObject("Java", "java.io.ByteArrayInputStream") />
		
		<!--- verify that the data is binary data (via try/catch), create an input stream --->
		<cftry>
			<cfset InputStream = InputStream.init(arguments.data) />
			
			<cfcatch>
				<cfthrow
					type="Alagad.Image.BinaryDataError"
					message="Error converting binary data to an image. Attribute data must be a binary representation of an image." />
			</cfcatch>
		</cftry>
	...
	<cffunction name="writeToBase64" access="public" output="false" returntype="string" hint="I return the image data in base64.">
		...
		<cfif IsDefined("arguments.quality")>
			<!--- the user wants to controll the quality of the image --->
			<cfreturn ToBase64(writeToBinary(arguments.format, arguments.quality)) />
		<cfelse>
			<!--- the user wants to use the defaults (or has a non-jpeg image) --->
			<cfreturn ToBase64(writeToBinary(arguments.format)) />
		</cfif>
		
	</cffunction>
	
	<!--- writeToBinary --->
	<cffunction name="writeToBinary" access="public" output="false" returntype="binary" hint="I return the image data in binary.">
		<cfargument name="format" hint="I am the format of the data to return." required="yes" type="string" />
		<cfargument name="quality" hint="I am set the compression quality when writing the image." required="no" type="numeric" />
		<cfset var OutputStream = CreateObject("Java", "java.io.ByteArrayOutputStream").init() /> 
	...
	<cffunction name="writeQualityToOutputStream" access="private" output="false" returntype="void" hint="I write the image to an outputstream at a specific quality setting.">
		...
		<cfif NOT ImageWriterIterator.hasNext()>
		...
		<cfloop condition="ImageWriterIterator.hasNext()">
			<cftry>
				...
				<!--- This writer worked, break out of the image writer loop --->
				<cfbreak>
				
				<cfcatch>
					<!--- do nothing -- this will cause the next image writer to be tried --->
				</cfcatch>
			</cftry>
		</cfloop>
	...
	<!--- scaleToBox --->
	<cffunction name="scaleToBox" access="public" output="false" returntype="void" hint="I scale the image to fit within a specific pixel width and height.">
		<cfargument name="maxWidth" hint="I am the pixel width to rescale to fit within." required="yes" type="numeric" />
		<cfargument name="maxHeight" hint="I am the pixel height to rescale to fit within." required="yes" type="numeric" />
		<cfset var initialHeight = getHeight() />
		<cfset var initialWidth = getWidth() />
		<cfset var targetAspect = arguments.maxWidth/arguments.maxHeight />
		<cfset var imageAspect = initialWidth/initialHeight />
		
		<cfif arguments.maxWidth LT 1>
			<cfthrow message="Invalid maxWidth attribute.  The maxWidth attribute must be greater than or equal to 1." />
		</cfif>
		<cfif arguments.maxHeight LT 1>
			<cfthrow message="Invalid maxHeight attribute.  The maxHeight attribute must be greater than or equal to 1." />
		</cfif>
		
		<cfif targetAspect GT imageAspect>
			<!--- we need to scale to fit the height --->
			<cfset scaleHeight(arguments.maxHeight)>
		<cfelse>
			<!--- we need to scale to fit the width --->
			<cfset scaleWidth(arguments.maxWidth)>
		</cfif>
	...
	<cffunction name="drawSimpleString" access="public" output="false" returntype="void" hint="I am a simple method to draw a string on the image.">
		...
		<cfset Graphics.drawString(JavaCast("string", arguments.string), javaCast("int", arguments.x), javaCast("int", arguments.y)) />	
	</cffunction>
