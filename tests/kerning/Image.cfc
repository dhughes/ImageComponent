...
Version History:
  ...
  04/21/2005	D.Hughes	2.16		Added paid enhancement for kerning
...
<cfcomponent displayname="Image" hint="I am a CFC which can be used to create and edit images." output="no">
	...
	<cfset variables.version.version = "2.14" />
	<cfset variables.version.releaseDate = "04/13/2005" />
	...
	<!--- CreateAttributedCharacter --->
	<cffunction name="createAttributedCharacter" access="private" output="false" returntype="any" hint="I convert a character from an advanced string to an attributed string">
		<cfargument name="AdvancedString" hint="I am the Advanced String to get the metrics of." required="yes" type="any" />
		<cfargument name="index" hint="I am the index of the char to create" required="yes" type="numeric" />
		<cfset var AttributedString =  CreateObject("Java", "java.text.AttributedString") />
		
		<!--- validate the advancedString --->
		<cfif Not IsAdvancedString(arguments.AdvancedString)>
			<cfthrow message="The advancedString attribute must be an Advanced String Object." />
		</cfif>
		
		<cfset AttributedString.init(Mid(arguments.AdvancedString.string, arguments.index, 1)) />
		
		<cfloop from="1" to="#ArrayLen(arguments.AdvancedString.attributes)#" index="x">
			<cfif arguments.AdvancedString.attributes[x].start LTE arguments.index AND arguments.AdvancedString.attributes[x].end GTE arguments.index>
				<cfset AttributedString.addAttribute(
					arguments.AdvancedString.attributes[x].attribute,
					arguments.AdvancedString.attributes[x].value
				) />
			</cfif>
		</cfloop>
		
		<cfreturn AttributedString />
	</cffunction>
	...
	<cffunction name="getStringMetrics" access="public" output="false" returntype="struct" hint="I return a structure of font metrics for the advanced string.">
		...
		<cfset var AttributedString = arguments.AdvancedString.attributedString />
		<cfset var xOffset = 0 />
		<cfset var LSB = 0 />
		<cfset var RSB = 0 />
		<cfset var advance = 0 />
		<cfset var glyphWidth = 0 />
		<cfset var kerning = 0 />
		...
		<cfset Iterator = AttributedString.getIterator() />
		...
		<!--- get the font metrics (all but width) --->
		...
		<!--- loop over each character in the itterator and draw it (with attributes) --->
		<cfloop from="0" to="#Len(arguments.AdvancedString.string) - 1#" index="i">
			<cfset Font = CreateObject("Java", "java.awt.Font").init(Iterator.getAttributes()) />
			<cfset GlyphVector = Font.createGlyphVector(Graphics.getFontRenderContext(), Mid(arguments.AdvancedString.string, i + 1, 1)) />
			
			<!--- get some of the glyph's attributes --->
			<cfset GlyphMetrics = GlyphVector.getGlyphMetrics(JavaCast("int", 0)) />
			<cfset LSB = GlyphMetrics.getLSB() />
			<cfset RSB = GlyphMetrics.getRSB() />
			<cfset advance = GlyphMetrics.getAdvance() />				
			<cfset glyphWidth = GlyphMetrics.getBounds2D().getWidth() />
						
			<!--- after we've draw the first character figure out what the kerning will be for the current character --->
			<cfif i IS NOT 0 AND arguments.AdvancedString.kerning[i] IS NOT 0>
				<cfset kerning = (abs(LSB + LSB) * (arguments.AdvancedString.kerning[i] / 100)) />
			<cfelse>
				<cfset kerning = 0 />
			</cfif>
			
			<!--- determine where the next glyph will be drawn --->
			<cfset xOffset = xOffset + kerning + LSB + glyphWidth + RSB />
			
			<!--- move to the next glyph in the glyph vector (vector meaning collection/array, not shape) --->
			<cfset Iterator.next() />
		</cfloop>
		
		<cfset metrics.width = ceiling(xOffset) />
	...
	<!--- hasKerning --->
	<cffunction name="hasKerning" access="private" output="false" returntype="boolean" hint="I indicate if any custom kerning has been applied to an advanced string">
		<cfargument name="AdvancedString" hint="I am the Advanced String to draw." required="yes" type="any" />
		<cfset var kerned = false />
		<cfset var x = 0 />
		
		<cfloop from="1" to="#ArrayLen(arguments.AdvancedString.kerning)#" index="x">
			<cfif arguments.AdvancedString.kerning[x] IS NOT 0>
				<cfset kerned = true />
			</cfif>
		</cfloop>
	...
	<cffunction name="drawString" access="public" output="false" returntype="void" hint="I draw the advanced string onto the image.">
		...
		<cfset var Font = 0 />
		<cfset var GlyphVector = 0 />
		<cfset var GlyphMetrics = 0 />
		<cfset var LSB = 0 />
		<cfset var RSB = 0 />
		<cfset var advance = 0 />
		<cfset var xOffset = arguments.x />
		<cfset var kerning = 0 />
		<cfset var glyphWidth = 0 />
		<cfset var styleIterator = 0 />
		<cfset var oldFill = 0 />
		<cfset var TextAttributes = CreateObject("Java", "java.awt.font.TextAttribute") />
		<cfset var StringMetrics = getStringMetrics(arguments.AdvancedString) />
		<cfset var exists = 0 />
		<cfset var transparent = CreateObject("Java", "java.awt.Color").init(JavaCast("float", 1), JavaCast("float", 1), JavaCast("float", 1), JavaCast("float", 0)) />
		<cfset var bgStartX = 0 />
		<cfset var bgThisColor = 0 />
		<cfset var bgNextColor = 0 />
		...
		<cfif hasKerning(arguments.AdvancedString)>
			<!--- before we can draw the text, we need to draw any text backgrounds which may be required!! --->
			<cfset bgStartX = xOffset />
			<cfset bgThisColor = transparent />
			<cfset bgNextColor = transparent />
			<cfloop from="0" to="#Len(arguments.AdvancedString.string) - 1#" index="i">
				<cfset Font = CreateObject("Java", "java.awt.Font").init(Iterator.getAttributes()) />
				<cfset GlyphVector = Font.createGlyphVector(Graphics.getFontRenderContext(), Mid(arguments.AdvancedString.string, i + 1, 1)) />
				...
				<!--- after we've drawn the first character figure out what the kerning will be for the current character --->
				<cfif i IS NOT 0 AND arguments.AdvancedString.kerning[i] IS NOT 0>
					<cfset kerning = (abs(LSB + LSB) * (arguments.AdvancedString.kerning[i] / 100)) />
				<cfelse>
					<cfset kerning = 0 />
				...
				<!--- note the old settings for the AIC --->			
				<cfset oldFill = Graphics.getPaint() />
	
				<!--- get this background --->
				<cfset exists = Iterator.getAttribute(TextAttributes.BACKGROUND) />
				<cfif IsDefined("exists")>
					<cfset bgThisColor = Iterator.getAttribute(TextAttributes.BACKGROUND) />
				<cfelse>
					<cfset bgThisColor = transparent />
				</cfif>
				
				<!--- get the next background --->
				<cfset Iterator.setIndex(JavaCast("int", i + 1)) />
				<cfset exists = Iterator.getAttribute(TextAttributes.BACKGROUND) />
				<cfif IsDefined("exists")>
					<cfset bgNextColor = Iterator.getAttribute(TextAttributes.BACKGROUND) />
				<cfelse>
					<cfset bgNextColor = transparent />
				</cfif>
				<cfset Iterator.setIndex(JavaCast("int", i)) />
							
				<cfset xOffset = xOffset + kerning + LSB + glyphWidth + RSB />
				
				<!--- is this background different from the next background? --->
				<cfif bgThisColor.hashCode() IS NOT bgNextColor.hashCode()>
					<!--- yes! draw a background from the bgStartX to the current bgStartX --->
					<!--- determine where the glyph will end --->
					<cfset Graphics.setPaint( bgThisColor ) />
					<cfset Graphics.fillRect(JavaCast("int", bgStartX), JavaCast("int", arguments.y - StringMetrics.ascent), JavaCast("int", xOffset - bgStartX), JavaCast("int", StringMetrics.ascent + StringMetrics.descent)) />
					
					
					<cfset bgStartX = xOffset />
				</cfif>
				
				<!--- reset the old settings for the AIC --->
				<cfset Graphics.setPaint(oldFill) />
			...
			<!--- re-get the itterator --->
			<cfset Iterator.setIndex(JavaCast("int", 0)) />
			<cfset xOffset = arguments.x />
			<cfset kerning = 0 />
			...
			<cfloop from="0" to="#Len(arguments.AdvancedString.string) - 1#" index="i">
				...
				<!--- draw the glyph's shape --->
				<cftry>
					<cfset Graphics.setPaint( Iterator.getAttribute(TextAttributes.FOREGROUND) ) />
					<cfcatch></cfcatch>
				</cftry>
				<cfset Graphics.fill(GlyphVector.getGlyphOutline(0, JavaCast("float", xOffset + kerning), JavaCast("float", arguments.y))) />
	...
	<cffunction name="createString" access="public" output="false" returntype="any" hint="I create and return a new Advanced String for advanced text formatting.">
		...
		<!--- add to the struct the string we're working with --->
		<cfset stringStruct.string = arguments.string />
		
		<!--- create the attributed string --->
		<cfset stringStruct.attributedString = CreateObject("Java", "java.text.AttributedString").init(JavaCast("string", arguments.string)) />

		<!--- add the kerning array --->		
		<cfset stringStruct.kerning = ArrayNew(1) />
		<cfif Len(arguments.string)>
			<cfset ArraySet(stringStruct.kerning, 1, Len(arguments.string) - 1, 0) />
		</cfif>
		<cfreturn stringStruct />
	</cffunction>
	
	<!--- IsAdvancedString --->
	<cffunction name="IsAdvancedString" access="private" output="false" returntype="boolean" hint="I indicate if the provided value is an advanced string object.">
		<cfargument name="AdvancedString" hint="I am the Advanced String to check." required="yes" type="any" />
		<cfset var valid = true />
		<!---
			previous versions of the AIC had an Advanced String object actualy be a java object.  As of 2.15 this has changed and is a struct.  However, I don't 
			want to break any implementations of the AIC so I'm leaving the type on Advanced String arguments as any and then verifying the data manually.
		--->
		
		<cfif NOT (
			IsStruct(arguments.AdvancedString) AND
			IsSimpleValue(arguments.AdvancedString.string) AND 
			IsObject(arguments.AdvancedString.attributedString) AND 
			IsArray(arguments.AdvancedString.kerning)
		)>
			<cfset valid = false />
		</cfif>
		<cfreturn valid />
	...
	<!--- setStringKerning --->
	<cffunction name="setStringKerning" access="public" output="false" returntype="void" hint="I set advanced text background color.">
		<cfargument name="AdvancedString" hint="I am the Advanced String to format." required="yes" type="any" />
		<cfargument name="Kerning" hint="The kerning to apply. Any numeric value between -100 and 400.  The default is 0.  0 indicates the standard kerning and will perform better than customized kerning." required="yes" type="numeric" />
		<cfargument name="Start" hint="The start character to apply the style to.  This will affect kerning to the right of this character." required="no" />
		<cfargument name="End" hint="The start character to apply the style to.  This will affect kerning to the right of this character." required="no" />
		
		<!--- set default values --->
		<cfif NOT IsDefined("arguments.start")>
			<cfset arguments.start = 0 />
		</cfif>
		<cfif NOT IsDefined("arguments.end")>
			<cfset arguments.end = ArrayLen(arguments.AdvancedString.Kerning) - 1 />
		</cfif>
		
		<!--- validate the kerning value --->
		<cfif arguments.Kerning LT -100 OR arguments.Kerning GT 400>
			<cfthrow message="Invalid kerning attribute.  The kerning attribute must be between -100 and 400." />
		</cfif>
		
		<!--- check to see if the kerning array has a length (meaning that we have a string which can be kerned) --->
		<cfif ArrayLen(arguments.AdvancedString.Kerning)>
			<!--- validate the start argument --->
			<cfif arguments.Start LT 0 OR arguments.Start GT ArrayLen(arguments.AdvancedString.Kerning) - 1>
				<cfthrow message="Invalid start attribute, #arguments.Start#.  The start attribute must be between 0 and one less than the length of the string being kerned." />
			</cfif>
			<!--- validate the end argument --->
			<cfif arguments.End LT 0 OR arguments.End GT ArrayLen(arguments.AdvancedString.Kerning) - 1>
				<cfthrow message="Invalid end attribute, #arguments.End#.  The end attribute must be between 0 and one less than the length of the string being kerned." />
			<cfelseif arguments.Start GT arguments.End>
				<cfthrow message="Invalid end attribute, #arguments.end#.  The end attribute must be greater than or equal to the start attribute, #arguments.start#." />
			</cfif>
		</cfif>
		
		<!--- add one to the start and end arguments to match up with the indexes in the kerning array --->
		<cfset arguments.start = arguments.start + 1 />
		<cfset arguments.end = arguments.end + 1 />
		
		<!--- set the kerning as instructed --->
		<cfset ArraySet(arguments.AdvancedString.kerning, arguments.start, arguments.end, arguments.kerning) />
	</cffunction>
	...
	<cffunction name="setStringFont" access="public" output="false" returntype="void" hint="I set advanced text font based on a provided font.">
		...
		<!--- we can't actually use the font object because when we're kerning the object we don't get the correct sizes, so we're going to manually add all the styles --->
		<cfswitch expression="#Font.getStyle()#">
			<cfcase value="1">
				<!--- bold --->
				<cfset setStringWeight(arguments.AdvancedString, "bold", arguments.start, arguments.end) />
			</cfcase>
			<cfcase value="2">
				<!--- italic --->
				<cfset setStringPosture(arguments.AdvancedString, "oblique", arguments.start, arguments.end) />
			</cfcase>
			<cfcase value="3">
				<!--- boldItalic --->
				<cfset setStringPosture(arguments.AdvancedString, "oblique", arguments.start, arguments.end) />
		...
		<!--- set the family --->
		<cfset setStringFamily(arguments.AdvancedString, Font.getFamily(), arguments.start, arguments.end) />
		
		<!--- set the size --->
		<cfset setStringSize(arguments.AdvancedString, Font.getSize(), arguments.start, arguments.end) />
	...
	<cffunction name="setTextAttribute" access="private" output="false" returntype="void" hint="I set a given text attribute to the provided value.">
		...
		<cfset stringLen = Len(arguments.AdvancedString.string) />
			
		<!--- validate start and end. --->
		<cfif arguments.start GT -1 AND arguments.end GT -1>
			<cfif arguments.start GT arguments.end>
				<cfthrow message="If the start and end attributes are provided they must both be greater than or equal to 0.  The end attribute must be greater than or equal to the start attribute.">
			</cfif>
			
			<!--- make sure start and end are both greater than or equal to 0 --->
			<cfif arguments.start LT 0 OR arguments.end LT 0>
				<cfthrow message="The start and end attributes must be greater than 0.">
			</cfif>
		...
		<!--- if the start and end are -1 then set the start to the start and the end to the end --->
		<cfif arguments.start IS -1 AND arguments.end IS -1>
			<cfset arguments.start = 0 />
			<cfset arguments.end = Len(arguments.AdvancedString.string) />
		</cfif>
		
		<!--- append the attribute --->
		<cfset arguments.AdvancedString.attributedString.addAttribute(arguments.attribute, arguments.value, arguments.start, arguments.end) />
	</cffunction>
